...
func handler(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	reqData := &model.RespondToAuthChallengeRequest{}
	err := apigwevent.ParseRequest(req, reqData)
	if err != nil {
		return apigwevent.BadRequest(err)
	}
	log.WithFields(log.Fields{"username": reqData.Username, "challenge": reqData.ChallengeName}).Info("User respond to challenge")

	respData, err := authSvc.RespondToAuthChallenge(reqData)
	if err != nil {
